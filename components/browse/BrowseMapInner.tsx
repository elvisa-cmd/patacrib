...
export interface BrowseMapInnerProps {
...
const USER_PIN_HTML = `
  <style>@keyframes bmi-ring{
    0%{transform:translate(-50%,-50%) scale(1);opacity:0.55;}
    100%{transform:translate(-50%,-50%) scale(2.6);opacity:0;}
  }</style>
  <div style="position:relative;width:20px;height:20px;">
    <div style="position:absolute;top:50%;left:50%;width:40px;height:40px;
      background:${C.blueA};border-radius:50%;animation:bmi-ring 2s infinite;"></div>
...
export default function BrowseMapInner({
  ...
  return (
    <div className="absolute inset-0">
      ...
      {/* Drag hint */}
      {showDragHint && (
        <div className="absolute bottom-20 right-3 z-[1000] font-sans text-[11px] text-muted bg-surface border border-border px-3 py-1.5 shadow-sm pointer-events-none">
          ✋ Drag to explore
        </div>
      )}
    </div>
